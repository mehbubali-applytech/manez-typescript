...
const allFinanceExecutivesData: IFinanceExecutive[] = [
  ...
  },
  {
    ...
    email: "david.wilson@medicareinnovations.com",
  ...
  },
  {
    ...
    email: "lisa.garcia@betatech.com",
...
interface FinanceExecutivesTableProps {
  status?: string;
  department?: string;
  company?: string;
  role?: string;
...
const FinanceExecutivesTable: React.FC<FinanceExecutivesTableProps> = ({
  status = "all",
  department = "all",
  company = "all",
  role = "all",
  ...
  const [executiveToEdit, setExecutiveToEdit] = useState<IFinanceExecutive | null>(null);
  ...
  const filteredData = useMemo(() => {
    let result = [...allFinanceExecutivesData];
    ...
    if (company && company !== "all") {
      result = result.filter(executive => {
        const executiveCompany = executive.company?.toLowerCase() || '';
        const filterCompany = company.toLowerCase();
        return executiveCompany === filterCompany;
      });
    }
    ...
    return result;
  }, [status, department, company, role, dateRange]);
  ...
  useEffect(() => {
    setPage(1);
  }, [status, department, company, role, dateRange, searchQuery]);
  ...
  const handleEditFinanceExecutive = (executive: IFinanceExecutive) => {
    setExecutiveToEdit(executive);
  ...
  const exportData = useMemo((): TableData => {
    ...
    return {
      headers,
      rows,
      title: `Finance Executives Export - ${sortedData.length} records`
    };
  }, [sortedData]);
  ...
  return (
    <>
              ...
              <Grid item xs={12} md={6}>
                <Box className="flex justify-end">
                  <DownloadButtonGroup
                    data={exportData}
                    options={{
                      fileName: `finance_executives_${new Date().toISOString().split('T')[0]}`,
                      includeHeaders: true,
                      pdfTitle: `Finance Executives Report - ${new Date().toLocaleDateString()}`
                    }}
                    variant="outlined"
                    size="small"
                    color="primary"
                            ...
                            <div className="flex flex-col items-center justify-center">
                              ...
                              <Typography variant="body1" className="text-gray-600 mb-2">
                                {filteredData.length === 0
                                  ? "No finance executives match your filters"
                                  : "No finance executives found with current search"}
                              </Typography>
                              ...
                              <TableCell className="table__icon-box">
                                <div className="flex items-center justify-start gap-2">
                                  ...
                                  <button
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleEditFinanceExecutive(row);
                                    }}
      ...
      {updateModalOpen && executiveToEdit && (
        <UpdateFinanceExecutiveModal
          open={updateModalOpen}
          setOpen={setUpdateModalOpen}
          financeExecutiveData={executiveToEdit}
