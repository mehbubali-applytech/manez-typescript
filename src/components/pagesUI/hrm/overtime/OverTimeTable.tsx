...
import { overTimeHeadCells } from "@/data/table-head-cell/table-head";
import Link from "next/link";
import Image from "next/image";
import { overtimeData } from "@/data/hrm/overtime-data";
import OverTimeEditModal from "./OverTimeEditModal";
import TableControls from "@/components/elements/SharedInputs/TableControls";
import DeleteModal from "@/components/common/DeleteModal";

const OverTimeTable = () => {
  const [modalOpen, setModalOpen] = useState(false);
  const [editData, setEditData] = useState<IOvertimeRecord | null>(null);
  ...
  const {
    ...
    handleSearchChange,
  } = useMaterialTableHook<IOvertimeRecord | any>(overtimeData, 10);
                  ...
                  >
                    <TableHead>
                      <TableRow className="table__title">
                        {overTimeHeadCells.map((headCell) => (
                          <TableCell
