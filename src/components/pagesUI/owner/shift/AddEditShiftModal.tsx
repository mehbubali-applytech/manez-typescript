...
import { IShift, IShiftForm, BreakTimeSlot, OFFICE_LOCATIONS, TIME_OPTIONS } from "./ShiftTypes";
...
interface AddEditShiftModalProps {
  ...
  editData?: IShift;
  onSave: (data: IShiftForm) => void;
}

const AddEditShiftModal: React.FC<AddEditShiftModalProps> = ({
  open,
  setOpen,
  editData,
  onSave,
  ...
  const onSubmit = async (data: IShiftForm) => {
    ...
    try {
      await onSave(data);
      toast.success(editData ? "Shift updated successfully!" : "Shift created successfully!");
      setOpen(false);
...
function calculateTotalBreakTime(breakSlots: BreakTimeSlot[]): number {
